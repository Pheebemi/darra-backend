...
export default function SellerProfilePage() {
  ...
  const handleSave = async () => {
    ...
    if (!brandName.trim()) { toast.error("Brand name cannot be empty"); return; }
    setSaving(true);
    try {
      const res = await fetch("/api/auth/profile/update", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ full_name: fullName, brand_name: brandName }),
      });
      if (!res.ok) {
        ...
        {loading ? (
          ...
          <Card>
            <CardContent className="p-6 space-y-5">
              ...
              <div>
                <Label htmlFor="brand_name" className="mb-1.5 block text-sm">Brand Name</Label>
                <Input
                  id="brand_name"
                  value={brandName}
                ...
                />
                <p className="mt-1 text-xs text-muted-foreground">This is your public store name</p>
              </div>
